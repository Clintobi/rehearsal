...
import { breakerPda, decodeBreaker } from "./guard";
...
let cache: { at: number; body: HaltBoard } | null = null;

export type HaltRow = {
  symbol: string; ticker: string;
  nasdaq: { halted: boolean; reason: string | null; at: string | null; resumed: boolean | null };
  breaker: { address: string; state: string; bandBps: number; exchangeHalted: boolean; haltReason: string; trips: number } | null;
};
export type HaltBoard = { cluster: string; feedItems: number; at: number; rows: HaltRow[] };

export async function haltBoard(): Promise<HaltBoard> {
  ...
  const body: HaltBoard = { cluster: BREAKER_CLUSTER, feedItems: items.length, at: Date.now(), rows };
