...
surfpool start -u "$MAINNET_RPC" --db ":memory:" --no-tui --no-deploy --no-studio -y > /tmp/surfpool.log 2>&1 &
...
solana program deploy onchain/target/deploy/rehearsal_guard.so --program-id onchain/target/deploy/rehearsal_guard-keypair.json -k onchain/keys/deployer.json -u $L | tail -1
